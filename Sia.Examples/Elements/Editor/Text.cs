...
internal sealed class TextLeaf : Text
{
    ...
    {
        var text = from <= 0 && to >= Length ? this
    ...
    internal static Text[] Split(string[] text, List<Text> target)
    {
        ...
        foreach (var line in text) {
            ...
            if (part.Count == TextConst.Branch) { target.Add(new TextLeaf([.. part], len)); part.Clear(); len = -1; }
        }
    ...
    internal static string[] Append(string[] text, string[] target, int from, int to)
    {
        ...
        for (int pos = 0, i = 0; i < text.Length && pos <= to; i++) {
            ...
            if (end >= from) {
                ...
                if (first) { target[^1] += line; first = false; } else target = [.. target, line];
            }
            pos = end + 1;
        }
        return target;
...
internal sealed class TextNode : Text
{
    ...
    internal static Text From(List<Text> children, int length = -1)
    {
        ...
        void Add(Text child)
        {
            if (child.Lines > maxChunk && child is TextNode n) { foreach (var c in n._children) Add(c); } else if (child.Lines > minChunk && (curLines > minChunk || curLines == 0)) { Flush(); chunked.Add(child); } else if (child is TextLeaf lf && curLines > 0 && curChunk[^1] is TextLeaf last &&
                                                                                                                                                                                                                               lf.Lines + last.Lines <= TextConst.Branch) { curLines += lf.Lines; curLen += lf.Length + 1; curChunk[^1] = new TextLeaf([.. last.Lines_, .. lf.Lines_], last.Length + 1 + lf.Length); } else { if (curLines + child.Lines > chunk) Flush(); curLines += child.Lines; curLen += child.Length + 1; curChunk.Add(child); }
        }
