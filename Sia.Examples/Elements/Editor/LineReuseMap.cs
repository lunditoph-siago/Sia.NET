...
{
    public static int[] Compute(ChangeSet changes, Text oldDoc, Text newDoc)
    {
        var oldLines = oldDoc.Lines;
        var map = new int[oldLines];
        if (changes.IsEmpty) {
            for (var index = 0; index < oldLines; index++) {
                map[index] = index;
            }
            return map;
        }
        ...
        changes.IterateChangedRanges((fromA, toA, fromB, toB) => {
            ...
            for (; oldPos < startLine && oldPos < oldLines; oldPos++) {
                map[oldPos] = oldPos + delta;
            }
            ...
            for (var offset = 0; offset < shared && oldPos < oldLines; offset++, oldPos++) {
                map[oldPos] = newStartLine + offset;
            }
            for (; oldPos <= endLine && oldPos < oldLines; oldPos++) {
                map[oldPos] = -1;
            }
