...
public class ChangeDesc
{
    ...
    public int NewLength {
        get {
            var r = 0;
            for (var i = 0; i < Sections.Length; i += 2) { var ins = Sections[i + 1]; r += ins < 0 ? Sections[i] : ins; }
            return r;
        }
    }
    ...
    public ChangeDesc InvertedDesc {
        get {
            ...
            for (var i = 0; i < Sections.Length;) { int len = Sections[i++], ins = Sections[i++]; s[i - 2] = ins >= 0 ? ins : len; s[i - 1] = ins >= 0 ? len : ins; }
    ...
    public int? MapPos(int pos, int assoc = -1, MapMode mode = MapMode.Simple)
    {
        ...
        for (var i = 0; i < Sections.Length;) {
            ...
            if (ins < 0) { if (endA > pos) return posB + (pos - posA); posB += len; } else {
