...
public partial record struct InputDevice(
    [Sia] string DeviceId,
    [Sia] bool IsConnected)
{
    public InputDevice() : this("default", true) { }
}

public partial record struct MouseState(
    [Sia] Vector2 Position,
    [Sia] Vector2 DeltaPosition)
{
    public MouseState() : this(Vector2.Zero, Vector2.Zero) { }
}

public partial record struct KeyboardState(
    [Sia] ImmutableHashSet<Key> PressedKeys)
{
    public KeyboardState() : this([]) { }
}

public partial record struct MouseButtonState(
    [Sia] ImmutableHashSet<MouseButton> PressedButtons)
{
    public MouseButtonState() : this([]) { }
}

public partial record struct ScrollState(
    [Sia] Vector2 ScrollDelta)
{
